...
# Session tokens mapped to their API token hash (simple in-memory store)
...
def create_session(api_token: str) -> str:
    ...
    _sessions[session_id] = _token_hash(api_token)
    return session_id


def validate_session(session_id: str, expected_token: str) -> bool:
    """Check if a session ID is valid for the expected API token."""
    stored_hash = _sessions.get(session_id)
    if not stored_hash:
        return False
